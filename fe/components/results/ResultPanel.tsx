      ...
      <div className="flex border-b border-surface-200 px-2">
        {visibleTabs.map((tab) => (
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-3 text-sm font-medium border-b-2 transition-all duration-200 ${
              activeTab === tab.key
    ...
    <>
      ...
      {hasData && (
        <button
          ...
          className="lg:hidden fixed bottom-20 right-4 z-40 w-14 h-14 rounded-full bg-gradient-to-r from-primary-500 to-primary-600 text-white shadow-xl shadow-primary-500/30 flex items-center justify-center active:scale-95 transition-transform"
          aria-label="Xem kết quả"
        >
          <span className="text-xl">{isSheetOpen ? "✕" : "📋"}</span>
          {!isSheetOpen && (
            <span className="absolute -top-1 -right-1 w-5 h-5 bg-accent-emerald rounded-full text-[10px] text-white font-bold flex items-center justify-center">
